...
def test_render_locales(config, journalist_app, test_journo, test_source):
    # We use the fixture to generate all our tables, but we don't use it during
    # the test because we need to inject the i18n settings (which are only
    # triggered during create_app
...
def test_col_process_successfully_unstars_sources(journalist_app, test_journo):
    source_1, _ = utils.db_helper.init_source()
    utils.db_helper.submit(source_1, 1)
    ...
    source_1 = Source.query.get(source_1.id)
    assert not source_1.star.starred
